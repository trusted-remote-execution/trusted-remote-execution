...
# Run a Rhai script via SSM with READ-ONLY policy.
# Uses the Rex-RO SSM document which hardcodes the readonly.cedar policy.
...
if [ $# -lt 1 ]; then
  ...
  echo ""
  echo "Examples:"
  echo "  $0 'let x = cat(\"/etc/hostname\"); x'"
...
# If script input is a file, read its contents
if [ -f "${SCRIPT_INPUT}" ]; then
  SCRIPT_CONTENT=$(cat "${SCRIPT_INPUT}")
else
...
echo "==> Running Rex (READ-ONLY) on ${INSTANCE_ID}..."
